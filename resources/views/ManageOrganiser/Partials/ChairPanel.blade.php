<div class="panel panel-success event">
    <div class="panel-heading" data-style="background-color: red; background-size: cover;">
        <div class="event-date">
            ...
            <img src="{{$chair->image}} " alt="" class="img">
        </div>
        <ul class="event-meta">
            <li class="event-title">
                <h4>
                {{-- <h4 title="{{{$program->title}}}" href="{{route('showEventDashboard', ['event_id'=>$program->id])}}"> --}}
                    {{{ Str::limit($chair->firstname." ".$chair->lastname , $limit = 75, $end = '...') }}}
                </h4>
            </li>
    ...
    <div class="panel-footer">
        <ul class="nav nav-section nav-justified">
            <li>
                <a href="" data-modal-id="update" data-href="{{route('showUpdateChair', ['organiser_id' => @$organiser->id, 'chair_id' => $chair->id  ])}}" class="loadModal" id="{{$chair->id}}"><i class="ico-edit"></i> @lang("basic.edit")</a>
            </li>
            <li>
                <a  data-modal-id="removeProgram"  id="{{$chair->id}}" onclick="popupRMV({{$chair->id}})"><i class="ico-remove"></i> Remove</a>
            </li>
