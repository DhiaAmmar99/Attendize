...
function listDelegate(val){ 
  ...
  jQuery.ajax({
        type: "GET",
        url: "https://preprodica.digitalresearch.ae/api/checkuser/" + val,
...
function sendMail(val, dl){
  ...
  val.forEach(element => {
    jQuery.ajax({
    type: "GET",
    url: "https://preprodica.digitalresearch.ae/api/send_email/registrations/"+element.id,
    dataType: 'jsonp',
    });
  ...
  dl.forEach(el => {
    ...
    jQuery.ajax({
    type: "GET",
    url: "https://preprodica.digitalresearch.ae/api/send_email/delegates/"+el.id,
    dataType: 'jsonp',
    });
  });
  });
...
function paymentStatus(event, id){
 ...
 var dataTab = {
    ...
    jQuery.ajax({
      type: "POST",
      data: dataTab,
      url: "https://preprodica.digitalresearch.ae/api/payment",
      success: function(data) {
        location.reload();
      }
    });
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('Shared.Layouts.Master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/backoffice/resources/views/ManageOrganiser/list.blade.php ENDPATH**/ ?>